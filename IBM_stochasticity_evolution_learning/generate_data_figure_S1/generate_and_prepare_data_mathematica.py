...
for gi in range(g0 + 1, gmax + 1):
    ...
    if gi % 1000 == 0:
        ...
        tic = time.perf_counter()
    if gi > 100000:
        distribution_k = [i.k for i in G]
        distribution_k_over_gen.append(distribution_k)

save_data_for_mathematica(data_folder, ngmin=100000, ngmax=None)
with open(data_folder + "/Mathematica/distk.json", 'w') as file:
    json.dump(distribution_k_over_gen, file)
